...
def calibrate_safety_policy(
    ...
    """
    Select the highest-coverage policy whose conservative validation
    error-rate upper bound is at most MAX_VALIDATION_UNSAFE_RATE.

    A Bonferroni-style correction protects against testing many candidate
    confidence/similarity policies on the same validation partition.
    """
...
def print_result(
    ...
    print()
    print(f"BGL-Only Few-Shot Baseline: {support_size} Labelled Incidents")
    print("-" * 60)
    ...
    print(
        ...
        f"{policy['validation_unsafe_upper_bound']:.4f}"
    )
    print()
    print("No abstention:")
    ...
    print("With BGL-only abstention:")
...
def main() -> None:
    ...
    validation_texts = texts(bgl_validation)
    test_texts = texts(bgl_test)
    y_validation = labels(bgl_validation)
    y_test = labels(bgl_test)

    ARTIFACT_DIR.mkdir(parents=True, exist_ok=True)

    all_results = {
        "experiment": "bgl_development_few_shot_selection",
        "training_domain": "BGL",
        "bgl_support_pool_incidents": len(bgl_support_pool),
        "bgl_validation_incidents": len(bgl_validation),
        "bgl_development_evaluation_incidents": len(bgl_test),
        "support_sizes": SUPPORT_SIZES,
        "target_split": "chronological_60_20_20",
        "runs": {},
    }

    print("BGL Development-Only Few-Shot Selection")
    print("=" * 28)
    print(f"BGL support pool incidents: {len(bgl_support_pool):,}")
    print(f"BGL validation incidents: {len(bgl_validation):,}")
    print(f"BGL development-evaluation incidents: {len(bgl_test):,}")
    ...
    for support_size in SUPPORT_SIZES:
        ...
        model_path = (
            ARTIFACT_DIR
            / f"bgl_development_support_{support_size}_seed_{SEED}.joblib"
        )
        ...
        all_results["runs"][str(support_size)] = {
            ...
            "ood_cutoff": ood_cutoff,
            "safety_policy": safety_policy,
            "development_evaluation_no_abstention": no_abstention,
            "development_evaluation_selective": selective,
    ...
    print()
    print(f"Saved BGL-only baseline results: {RESULTS_PATH}")
